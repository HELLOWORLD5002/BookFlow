...
public class ReturnServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (studentNumber == null) { response.sendRedirect(request.getContextPath() + "/pages/login.jsp"); return; }
        ...
        if (t != null && !t.isReturned()) {
            ...
            try {
                ...
                if (book != null) bookDAO.updateStock(t.getBookNo(), book.getStock() + 1);
            } catch(Exception ex) {}
            double fine = t.computeOverdueFine();
            if (fine > 0) {
                Penalty p = new Penalty(t.getStudentName(), t.getStudentNumber(), t.getBookName(), "Overdue Fine", fine);
                penaltyDAO.insert(p);
            }
        }
